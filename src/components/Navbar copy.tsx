            ...
            <div className="container mx-auto flex justify-between items-center">
                ...
                <div className="flex items-center">
                    {navItems.map((item) => item.dropdown && item.list ? (
                        ...
                        >
                            <Link href={item.href} className={`px-4 py-1 flex items-center space-x-2 transition ${basePath === item.href ? `text-[#f2b265]` : 'text-white'}`}>
                                {item.label}
                                <FaPlus className={`ms-1 transition ${basePath === item.href ? `text-[#f2b265]` : openDropdown === item.label && `text-[#f2b265]`} `} aria-hidden="true" />
                            </Link>
                                    ...
                                    >
                                        {item.list.map((listPage) => (
                                            <li key={listPage.label}>
                                                <Link href={item.href + listPage.href} className={`block px-4 py-2 items-center space-x-2 transition ${basePath === item.href + listPage.href ? `text-[#f2b265]` : 'text-white'}`}>
                                                    {listPage.label}
                                                </Link>
                                            </li>
                                        ))}
                        ...
                        <Link key={item.label} href={item.href} className={` px-4 py-1 flex items-center space-x-2 transition hover:text-[#f2b265] ${basePath === item.href ? `text-[#f2b265]` : 'text-white'}`}>
                            {item.label}
                        </Link>
                    ))}
                </div>
